...
static f64 g_ElapsedTime;

void ProcessEvents(GameContext *game)
{
	SDL_Event event;
	...
	static u64 curr_time;

	if(prev_time == 0)
		prev_time = SDL_GetPerformanceCounter();

	curr_time     = SDL_GetPerformanceCounter();
	g_ElapsedTime = (f64)(curr_time - prev_time) / (f64)SDL_GetPerformanceFrequency();
	prev_time     = curr_time;

	game->accumulator += g_ElapsedTime;
	...
	while(SDL_PollEvent(&event))
	{
		...
		{
			// Get Scaling Factor
			SDL_RenderSetViewport(game->renderer, &(SDL_Rect){0, 0, WINDOW_W, WINDOW_H});
		}
	}
}
